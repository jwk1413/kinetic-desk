...
(async()=>{
 ...
 build.onLoad({filter:/.*/,namespace:'stub'},args=>({contents:args.path==='shapes'?'export const invalidateSizeCaches=()=>{}; export const prepareShadowBuffer=()=>{};':`export class DoublePendulumObject { origin={x:350,y:360}; visibleBox(){return{x:330,y:340,width:40,height:60,pivotX:350,pivotY:360}} layout(){} applyPhysics(){} consumeWindowShift(){return {x:0,y:0}} dispose(){} hitTest(){return null} update(dt){globalThis.simTime+=dt} draw(){globalThis.draws++} }`,loader:'js'}));
 }}]});
 function run(hz,fps,paused,bench,mouse,jitter=false) {
  ...
  sandbox.window={innerWidth:700,innerHeight:720,devicePixelRatio:2,addEventListener:(k,fn)=>listeners[k]=fn,desk:{getState:()=>undefined,onState:fn=>stateListener=fn,ready:()=>stateListener(state),setClickThrough(){},moveWindowBy(){},setObjectBox(){}}};
